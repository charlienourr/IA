...
public class Jeu {
    ...
    public boolean legal(Domino l){
        // le domino est-il placÃ© dans la grille?
    ...
    /**
     * Retourne l'ensemble des dominos qui peuvent Ãªtre posÃ©s sur le plateau selon si le joueur joue
    ...
    public boolean estTerminee(){
        if (it%2==0){ // c'est Ã  ligne de jouer
            for (int i=0;i<n-1;i++){
                for (int j=0;j<n;j++){
                    if (libre[i][j] && libre[i+1][j])
                        return false;
    ...
    public static boolean estTermine(boolean[][] libre, int role){
        int n= libre.length;
        if (role==LIGNE){ // c'est Ã  ligne de jouer
            for (int i=0;i<n-1;i++){
                for (int j=0;j<n;j++){
                    if (libre[i][j] && libre[i+1][j])
                        return false;
    ...
    public int evaluation(int role){
        return getActionsPossibles(role).size()-getActionsPossibles((role+1)%2).size();
    }
